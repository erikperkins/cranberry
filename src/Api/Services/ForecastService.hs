...
data ForecastService = ForecastService { _redisService :: Snaplet RedisDB }
redisService :: Lens' ForecastService (Snaplet RedisDB)
redisService = lens _redisService (\a b -> a { _redisService = b })

forecastServiceInit :: Snaplet RedisDB -> SnapletInit b ForecastService
forecastServiceInit redisSnaplet = makeSnaplet "forecast" "Forecast Service" Nothing $ do
...
forecastEndpoint = do
  ...
  redisKeys <- runRedisDB redisService $ do keys "en:[0-9]*"
  key:_ <- case redisKeys of
    Left _ -> return []
    Right ks -> return . reverse . sort $ ks
  values <- runRedisDB redisService $ do hgetall key
